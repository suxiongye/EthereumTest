...
contract DataAccessObject{
    ...
    //storaget the requester => request object
    ...
    //the data capability
    string capability;
    //the visit number of requester
    mapping(address => uint) public numberOfAccess;
    
    //here should consider how to provide the token to requester and add visit timess
    
    function DataAccessObject(address pro, address dataM, string cap){
        provider = pro;
     ...
     */
    function getCapability() returns(string){
        //if the request is confirmed or sender is the provider
        if(isRequestConfirm(msg.sender) || msg.sender == provider){
            numberOfAccess[msg.sender]++;
            return capability;
        }
        return;
    }
     ...
     */
     function setCapability(string cap) isDataManagement{
         capability = cap;
     }
    /**
     ...
     * Add request to list
     */
    function addRequest(address requester, string information) isDataManagement returns(bool){
...
contract AccessManagement{
    ...
    */
    function createDataAccess(bytes32 dataName, address provider, string capability) returns(bool){
        if(isDataAccessExist(dataName)) return false;
        //create the data access object
        dataAccessList[dataName] = new DataAccessObject(provider, msg.sender, capability);
...
contract DataShareManagement{
    ...
    /**
      ...
      */
     function createData(bytes32 daNa, string intro, string cap) isTheUserRegister returns(address){
         //create data object
         address dataObject = dataRegister.addData(daNa, intro, msg.sender);
         if(dataObject == initAddress) return;
         //init the access control
         accessManagement.createDataAccess(daNa, msg.sender, cap);
        ...
        */
         ...
         */
          ...
          */
           ...
           */
          function requestData(bytes32 dataName, string information) isTheUserRegister returns(bool){
           ...
           */
            ...
            */
            ...
            /**
             * Get visit times of the data capability
             */
             function getDataVisitTime(bytes32 dataName, address requester) returns(uint){
                 DataAccessObject accessObject = (DataAccessObject)(getDataAccessByName(dataName));
                 return accessObject.numberOfAccess(requester);
             }
             
             /**
              * Set data capability
              */
              function setDataCapability(bytes32 dataName, string cap) returns(bool){
                  DataAccessObject accessObject = (DataAccessObject)(getDataAccessByName(dataName));
                  if(accessObject.provider() != msg.sender) return false;
                  accessObject.setCapability(cap);
                  return true;
              }
}